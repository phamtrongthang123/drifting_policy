#!/bin/bash
#SBATCH --job-name=ddpm-can-lowdim
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=32
#SBATCH --time=0-06:00:00
#SBATCH --output=slurm_logs/ddpm_can_lowdim_%j.out
...
echo '=== DDPM Baseline: Can Lowdim (200 epochs) ==='
...
python train.py \
    --config-dir=. \
    --config-name=ddpm_can_lowdim.yaml \
    training.seed=42 \
    training.device=cuda:0 \
    training.resume=false \
    hydra.run.dir='data/outputs/\${now:%Y.%m.%d}/\${now:%H.%M.%S}_ddpm_can_lowdim_baseline'
"
