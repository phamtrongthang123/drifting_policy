#!/bin/bash
#SBATCH --job-name=bp-sweep
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=24
#SBATCH --time=2-00:00:00
#SBATCH --output=slurm_logs/train_drifting_blockpush_lowdim_sweep_%j.out
#SBATCH --partition=agpu,vgpu
#SBATCH --constraint="1a100|1v100"
#SBATCH --exclude=c2110

# Usage: sbatch --export=GEN_PER_LABEL=4 scripts/slurm_train_drifting_blockpush_lowdim_sweep.sh
...
echo '=== Launching Drifting Policy Training (BlockPush Lowdim, gen_per_label=${GEN_PER_LABEL}) ==='
python train.py \
    --config-dir=. \
