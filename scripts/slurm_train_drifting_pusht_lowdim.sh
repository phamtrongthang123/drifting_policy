#!/bin/bash
#SBATCH --job-name=train-drifting-pusht-lowdim
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=24
#SBATCH --time=2-00:00:00
#SBATCH --output=slurm_logs/train_drifting_pusht_lowdim_%j.out
...
echo '=== Launching Drifting Policy Training (PushT Lowdim) ==='
python train.py \
    --config-dir=. \
    --config-name=drifting_pusht_lowdim.yaml \
    training.seed=42 \
